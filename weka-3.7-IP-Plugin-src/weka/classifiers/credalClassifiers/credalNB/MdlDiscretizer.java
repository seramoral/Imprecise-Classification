...
class MdlDiscretizer {
	...
	MdlDiscretizer(ArrayList<Double> SuppliedFeatureValues, ArrayList<Integer> SuppliedClassValues, int suppliedNumClasses)
	{
		...
		acceptedCutPoints=new ArrayList<Double>();
		...
		if (!possibleCutPoints.isEmpty()){
			...
			//IF CUT POINTS FOUND
			if (!acceptedCutPoints.isEmpty())
				Collections.sort(acceptedCutPoints);	
		}
	}
	...
	private void computePossibleCutPoints(){
		...
		for (int i = 1; i < pairVector.length; i++) {
			...
			if (((PreviousClassSet.size()) > 1 | (CurrentClassSet.size()) > 1)  | 
					(!PreviousClassSet.get(0).equals( CurrentClassSet.get(0) )) )
			{
	...
	/**Discretizes the variable and instantiates acceptedCutPoints; called from the constructor.
	 * 
	 */
	private boolean recursiveMDLDiscretization(int lowerIdx,int upperIdx) {
		...
		{
			acceptedCutPoints.add(currentBestCutPoint);
			//==CUT POINT DECIDED
			...
			while (pairVector[cutPointIdxInPairVector].getFeatureValue().equals( pairVector[cutPointIdxInPairVector+1].getFeatureValue() ))
				{
